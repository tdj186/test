...
//			var cacheTTL = 86400;
			var cacheTTL = 0;
			var hostUrl = 'http://api.jifupai.com';
			return ({
				/* Get/Set cache TTL */
				getTTL: function() {
					return cacheTTL;
				},
				setTTL: function(cacheTime) {
					cacheTTL = cacheTime;
				},
				$get: ['$q', '$http', function($q, $http) {
					...
					return {
						...
						_get: function() {
							return $http.get.apply(this, arguments)
						},
						_post: function() {
							return $http.post.apply(this, arguments)
						},
						_put: function() {
							return $http.put.apply(this, arguments)
						},
						_delete: function() {
							return $http.delete.apply(this, arguments)
						},
						_jsonp: function() {
							return $http.jsonp.apply(this, arguments)
						},
						_head: function() {
							return $http.head.apply(this, arguments)
						},
						...
						get: function(url, succFun) {
							__ajax($http.get, url, succFun)
						},
						delete: function(url, succFun) {
							__ajax($http.delete, url, succFun)
						},
						post: function(url, sendBody, succFun) {
							__ajax($http.post, url, succFun, sendBody)
						},
						put: function(url, sendBody, succFun) {
							__ajax($http.put, url, succFun, sendBody)
						},

						//  Triditional style 
//						cGet: function(url, succFun, cacheTime) {
						cGet: function(url, succFun) {
//							var ttl = (cacheTime === undefined) ? cacheTTL : cacheTime;
//
//							if (self.cache[url] !== undefined && ttl >= Math.floor(Date.now() / 1000) - self.cache[url][0]) {
//								succFun(self.cache[url][1]);
//								return;
//
//							}
							//$http.get(url).
						...
						//post sy
						cPost: function(url, sendBody, succFun) {
